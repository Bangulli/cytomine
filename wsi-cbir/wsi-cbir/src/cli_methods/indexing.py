...
def calculate_embedding_for_image(index, path, filename, image_id, image_meta=False):
    ...
    with tc.autocast(device.split(':')[0], precision), tc.no_grad():
        tc.cuda.empty_cache()
        embedding, _, sGP = calculate_embedding(precision, factory, device, image_path, CYTOMINE_CONFIG['target_mpp'], int(patch_size), int(patch_size), CYTOMINE_CONFIG['remove_bg'], transforms, patch_encoder, slide_encoder)
...
def calculate_embedding(precision, factory, device, image, target_mpp, patch_size, patch_stride, remove_bg, transforms, patch_encoder, slide_encoder):
