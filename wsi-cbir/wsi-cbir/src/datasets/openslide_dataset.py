...
class OpenSlideDataset(wsi.WholeSlide):
    """
    ...
    def __init__(self,
        ...
        try:
            self.mpp = (image_slide.properties[openslide.PROPERTY_NAME_MPP_X]*image_slide.level_downsamples[self.resolution_level], image_slide.properties[openslide.PROPERTY_NAME_MPP_Y]*image_slide.level_downsamples[self.resolution_level])
        except:
            self.mpp = (0.5, 0.5) # Default X20
    ...
    def calculate_upper_left_corners(self, image_slide : openslide.OpenSlide, mask_slide : openslide.OpenSlide=None):
        height = image_slide.level_dimensions[0][1] # NOTE: x, y in OpenSlide (instead of y, x in NumPy or PyTorch)
        width = image_slide.level_dimensions[0][0] # NOTE: x, y in OpenSlide (instead of y, x in NumPy or PyTorch)
    ...
    def _ensure_image_is_open(self):
        if self.image_slide is None:
            self.image_slide = openslide.open_slide(self.wsi_path)
