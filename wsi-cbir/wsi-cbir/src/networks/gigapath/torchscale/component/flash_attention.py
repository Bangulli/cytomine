...
# I have no idea why this works or why the problem appears at all, but this fixes the issue that on docker the interpreter cant find flash attention after installation
# --- flash-attn compat shim (works across FA 2.6+ and older) ---
try:
    # Newer FA exposes the canonical entrypoint here
    from flash_attn.flash_attn_interface import flash_attn_func
except Exception:
    try:
        # Some builds still expose at top-level
        from flash_attn import flash_attn_func
    except Exception:
        flash_attn_func = None  # let callers assert or fallback gracefully
# ----------------------------------------------------------------
