...
def make_shorthand(nme):
    shorthand = ''
    for section in nme.split('+'):
        for subword in section.strip().split(' '):
            shorthand += subword[0]
        shorthand += '+'
    if not shorthand.endswith('+'): return shorthand
    else: return shorthand.removesuffix('+')
    
def get_name(lst):
    return make_shorthand(make_name_from_list(lst))
...
class Index:
    ...
    def search_subset(self, query, k, ids):
        ...
        # params.sel = faiss.IDSelectorBatch(self.index.ntotal, faiss.swig_ptr(np.expand_dims(mask, 0)))
        dists, idxs = self.index.search(query, k, params=params)
        dists, idxs = dists.squeeze().tolist(), idxs.squeeze().tolist()
        print('dist', dists, 'idx', idxs)
        ids = map(self.idx2id_mapping.__getitem__, [str(i) for i in idxs])
        fns = map(self.idx2fn_mapping.__getitem__, [str(i) for i in idxs])
        return ids, dists, fns
    ...
    def filter_metadata(self, conditions, ids=None):
        ...
        print(f'== {len(subset)} samples fulfill the filter conditions, filtration took {time.time()-ct} seconds')
        return subset
        
    def add(self, samples, ids, meta=None): # samples are the embeddings arrays, ids are the corresponding image paths
    ...
    def add_dir(self, dir):
        ...
        return self.add(np.stack(embs), ids)
